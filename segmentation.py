...
class Segmentation():
    ...
    def manual(self, *echos : Echo, returnPoints : bool = False, boundary : str = 'all', saveFig : bool = False, phase : str = '', outputDir : str = "output") -> dict: # perform manual segmentation
        dp = {}
        for echo in echos:
            if not echo.segment:
                continue
            ...
            if len(ds.pixel_array.shape) == 4:
                ...
                img_gray = ds.pixel_array[echo.framenr,:,:]

            if boundary in ('endocard','all'):
                plt.imshow(img_gray, cmap="gray", aspect='auto')
                plt.title(f"Segment the endocardium ({echo.view} at {phase})")
            ...
            if boundary in ('epicard','all'):
                plt.imshow(img_gray, cmap="gray")
                plt.title(f"Segment the epicardium ({echo.view} at {phase})")
            ...
            plt.imshow(img_gray, cmap="gray")
            name_map = {"ES": "End systole", "ED": "End diastole"}
            plt.title(f"{echo.view} {name_map[phase]}")
            if boundary in ('endocard','all'):
                plt.scatter(coordinates_input_in[:,0],coordinates_input_in[:,1], facecolors='none', edgecolors=colorscheme["cyan"])
            if boundary in ('epicard','all'):
                plt.scatter(coordinates_input_out[:,0],coordinates_input_out[:,1], facecolors='none', edgecolors=colorscheme["magenta"])
            if saveFig:
                plt.savefig(os.path.join(outputDir,f"segmentation_{echo.view}_{phase}"))
            plt.close()

            if boundary == 'all':
                echo.datapoints2D = {"epicard": dp[echo.view + '_epicard'], "endocard": dp[echo.view + '_endocard']}
            else:
                echo.datapoints2D = {boundary: dp[echo.view + '_' + boundary]}
    ...
    def load_txt(filename : str ="Datapoints 2D", outputDir : str = "output"):
        filename = filename+".txt" if filename.split('.')[-1] != "txt" else filename
        ...
        for i,j in zip(idx[:-1],idx[1:]):
            lines_split = lines[i].split() # example = 'View AP2CH of epicard' -> ['View', 'AP2CH', 'of', 'epicard']
            bound = lines_split[1]
            view  = lines_split[-1]

            if bound not in datapoints.keys():
                datapoints[bound] = {view: np.array([ast.literal_eval(iline) for iline in lines[(i+1):j]])}
            else:
                datapoints[bound][view] = np.array([ast.literal_eval(iline) for iline in lines[(i+1):j]])
...
#     def combine_echos(self, *echos : Echo, tilt_range : tuple = (-40,40), tilt_steps : int = 101):
...
#                 RotMatrix   = R.from_euler('z', angle, degrees=False).as_matrix()[:2,:2]
#                 for boundary, datapoints2D in echo.datapoints2D.items():
...
#         tilt_angles_x  = np.linspace(-20,0,15) # Angle range for tilting the PSAX view
#         tilt_angles_y  = np.linspace(-20,20,15) # Angle range for tilting the PSAX view
#         rot_angles     = np.linspace(-20,20,15) # Angle range for rotating the PSAX view around its normal axis
#         transl_steps   = np.linspace(0,np.min(np.array(echoAP2CH.Bspline3D["endocard"].evalpts)),15) # vertical displacement/translation steps of the PSAX view
...
#     @staticmethod
#     def load_txt(filename : str ="Datapoints 3D", inputDir : str = "input", keepSlices=True):
...
#             points = np.array([ast.literal_eval(iline) for iline in lines[(i+1):j]])
#             if len(datapoints[bound]) == 0:
#                 datapoints[bound] = points
#             else:
#                 datapoints[bound] = np.r_[ datapoints[bound], points ]
